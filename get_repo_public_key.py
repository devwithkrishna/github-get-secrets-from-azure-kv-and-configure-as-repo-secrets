...
def get_repository_public_key(organization: str, repository_name: str):
	...
	if response_code == 200:
		print(
			f'Public key for {repository_name} retrieved from {organization} Org Successfully at {current_ist_time()}')
	else:
		print(
			f'Failed to retrieve public key for {repository_name} retrieved from {organization} Org at {current_ist_time()}')
...
def get_repository_public_key_id(organization: str, repository_name: str):
	...
	if response_code == 200:
		print(
			f'Public key id for {repository_name} retrieved from {organization} Org Successfully at {current_ist_time()}')
	else:
		print(
			f'Failed to retrieve public key id for {repository_name} retrieved from {organization} Org at {current_ist_time()}')
